...
*   **SSH Encryption Overhead (Simulation Caveat):** The relay transfer uses SSH pipes (`minikube ssh`) to simulate DTN/CCSDS store-and-forward. SSH's ChaCha20-Poly1305 encryption causes ~5-10% CPU overhead per hop that does not exist in real DTN RF hardware. When collecting thesis timing data, note that `relay_transfer.sh` reports wall-clock transfer times that include this encryption overhead. The actual transfer time over a physical ISL would be lower. Disabling SSH encryption is not feasible with Minikube's key management, so this overhead should be recorded as a known constant offset in the experimental results.