...
class FileTransferServicer(checkpoint_transfer_pb2_grpc.FileTransferServicer):
    ...
    # ==============================================================================
    # 2.2 ORCHESTRATORE LOCALE (Buildah + Kube) - Traduzione del vecchio Bash
    # ==============================================================================
    def rebuild_and_deploy(self, tar_path):
        print("🔨 [BUILDAH] Ricostruzione immagine in corso...")

        # Logica di Buildah tradotta dallo script originario
        build_script = f"""
        ...
        # Patch JSON basata sullo script bash originale
        patch_json = {
            "spec": {
                "template": {
                    "spec": {
        ...
        print("⏳ [K8S] In attesa del riavvio del Pod...")
        time.sleep(10)  # Pausa tattica per K8s

        # Cerca il nuovo pod e invia il segnale Wake Up
        pod_name = subprocess.check_output(
            f"kubectl get pod -l app=space-mission --field-selector spec.nodeName={NODE_NAME},status.phase=Running -o jsonpath='{{.items[0].metadata.name}}'",
            shell=True).decode('utf-8').strip()
        subprocess.run(f"kubectl exec {pod_name} -c sidecar-guardian -- touch /tmp/landed", shell=True)
        print("🎉 [ORCHESTRATOR] Migrazione completata localmente!")
...
def execute_migration_sender(target_node):
    ...
    # Trova il pod locale e freezalo
    pod_name = subprocess.check_output(
    ...
    proxy_proc.terminate()  # Spegne il proxy
    ...
    print(f"📦 [MITTENTE] Checkpoint creato: {checkpoint_path}")

    # --- NUOVA LOGICA DI RADAR KUBERNETES ---
    print(f"📡 [MITTENTE] Calcolo coordinate IP dell'Agente su {target_node}...")
    # Interroghiamo K8s per scoprire l'IP del Pod Agente sul nodo di destinazione
    ...
    # Invia il flusso continuo
    status = stub.StreamCheckpoint(generate_chunks(checkpoint_path))
    print(f"🏁 [MITTENTE] Trasferimento concluso! Esito: {status.message}")
